	...
	<button class="button_back">← Tilbage</button>
      ...
      <!--F O O T E R _____ S T A R T-->
        ...
        <div class="site-info">
	  	...
	  	<script>
		"use strict";
	let personlig_ydelse;
  document.addEventListener("DOMContentLoaded", getJson)


      async function getJson() {
        console.log("id er", <?php echo get_the_ID() ?>);
        let jsonData = await fetch(`http://bellakezia.dk/kea2/eksamen/cwc/wp-json/wp/v2/personlig-ydelse/<?php echo get_the_ID() ?>`);
        personlig_ydelse = await jsonData.json();
        vis(personlig_ydelse);
      }
      
      function vis(personlig_ydelse) {
 		document.querySelector(".overskrift_h2").innerHTML = personlig_ydelse.overskrift_h2;
		document.querySelector(".lang_beskrivelse").innerHTML = personlig_ydelse.lang_beskrivelse;
		document.querySelector(".overskrift_h3").innerHTML = personlig_ydelse.overskrift_h3;
